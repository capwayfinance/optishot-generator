...
function buildSuggestions(prompts) {
  plist.innerHTML='';
  ['Suggestion A','Suggestion B','Suggestion C'].forEach((lbl,i) => {
    if (!prompts[i]) return;
    const d = document.createElement('div');
    d.className = 'popt';
    d.innerHTML = `<span class="popt-tag">${lbl}</span><span class="popt-txt">${prompts[i]}</span><div class="pchk"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg></div>`;
    d.onclick = () => {
      document.querySelectorAll('.popt').forEach(x=>x.classList.remove('sel'));
      d.classList.add('sel'); selPrompt=prompts[i]; freePrompt.value='';
    };
    plist.appendChild(d);
  });
}
